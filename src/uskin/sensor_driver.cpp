...
* @class: ForceSensor;
*************************************************************************/
Uskin::Uskin(int devIndex_, int canIndex_): devType(VCI_USBCAN2) {
  devIndex = devIndex_;
  canIndex = canIndex_;
  status = 0;
  init_dev();
...
Uskin::Uskin(): devType(VCI_USBCAN2), devIndex(0), canIndex(0) {
  status = 0;
}
...
int Uskin::read_force(int forceMat[2][8][3], int timestamp[2][8]) {
